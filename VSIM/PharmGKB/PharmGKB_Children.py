...
NumChild = sys.argv[2]
fileName = "./PharmGKB/PhG_"+fileName1

if (os.stat(fileName).st_size == 0):
    print('File is empty! There is no match with PharmGKB')

else:
    Data = pd.read_csv("./PharmGKB/PhG_"+fileName1, header=None, sep="\t")
    phData = pd.DataFrame(
    ...
    report2.to_csv("./PharmGKB/temp/" + str(fileName1) + ".txt", sep="\t")
    finalRep = pd.read_csv("./PharmGKB/temp/" + str(fileName1) + ".txt", header=None, sep="\t")
    finalRep = finalRep.groupby([0])[1].count() / int(NumChild)

    finalRep.to_csv("./PharmGKB/temp/" + str(fileName1) + ".txt" , sep="\t")

    takeVarInfo = phData.drop_duplicates(subset='PharmPhenotypesHit', keep="first")

    finalRep = pd.read_csv("./PharmGKB/temp/" + str(fileName1) + ".txt" , sep="\t",names=['PharmPhenotypesHit','PhGKPLikelihood'])
    ...
    New.to_csv("./ChildResults/FinalPhG_" + str(fileName1), index=False, sep="\t")
