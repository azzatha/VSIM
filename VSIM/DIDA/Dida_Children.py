...
if (os.stat(fileName).st_size == 0):
    ...
    # print(GidIndv)
    GidIndv.to_csv("./DIDA/didaTimp/childGeneIDCount.txt", sep="\t")

    GidIndv2 = pd.read_csv('./DIDA/didaTimp/childGeneIDCount.txt', sep="\t", header=None)
    ...
    # print(GidIndv2)
    finalRep.to_csv("./DIDA/didaTimp/" + str(fileName1) + ".txt" , sep="\t")
    finalRep = pd.read_csv("./DIDA/didaTimp/" + str(fileName1) + ".txt", sep="\t",
    ...
    # print (didaData)
    orginalGenesCount = pd.read_csv('./DIDA/OriginalCount.txt', delim_whitespace=1, header=None)
    ...
    DidaData = pd.read_csv('./db/DIDA_Digenic_comb.csv', sep="\t")
    ...
    FileWithAllinfo.to_csv("./ChildResults/FinalDi_" + str(fileName1), index=False, sep="\t")
