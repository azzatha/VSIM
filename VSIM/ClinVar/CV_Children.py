...
NumChild = sys.argv[2]
fileName = "./ClinVar/CV_"+fileName1
...
if (os.stat(fileName).st_size == 0 or ('OMIM:' not in open(fileName).read())):
    ...
    omimMode = pd.read_csv('./ClinVar/omim_mode.txt', header=None, delim_whitespace=1)
    omimMode.columns = ['OMIM', 'Status']
    omimMode['OmimID'] = omimMode['OMIM'].str[5:]

    data = pd.read_csv("./ClinVar/CV_"+fileName1, header=None, delim_whitespace=1)
    ...
    report2.to_csv("./ClinVar/CVTimp/"+ str(fileName1)+".txt", sep="\t")
    finalRep = pd.read_csv("./ClinVar/CVTimp/"+ str(fileName1)+".txt", header=None, sep="\t")
    finalRep = finalRep.groupby([0, 1])[2].count() / int(NumChild)
    finalRep.columns=['CV_Class','OmimID','CVLikelihood']
    finalRep.to_csv("./ClinVar/CVTimp/"+ str(fileName1)+".txt", sep="\t")

    finalRep2 = pd.read_csv('./ClinVar/CVTimp/'+ str(fileName1)+".txt", sep="\t",names=['CV_Class','OmimID','CVLikelihood'])
    ...
    del New['CLNDN']
    del New['Link']
    New.to_csv("./ChildResults/FinalCV_" + str(fileName1), index=False, sep="\t")