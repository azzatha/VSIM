...
NumChild = sys.argv[2]
fileName = "./GWAS/GW_"+fileName1

if (os.stat(fileName).st_size == 0):
    print('File is empty! There is no match with GWAS')

else:
    GwasOrgData = pd.read_csv("./GWAS/GW_"+fileName1, header=None, delim_whitespace = 1)
    ...
    report2.to_csv("./GWAS/gwasTimp/" + str(fileName1) + ".txt", sep="\t")

    finalRep = pd.read_csv("./GWAS/gwasTimp/" + str(fileName1) + ".txt", header=None, sep="\t")
    finalRep = finalRep.groupby([0])[1].count() / int(NumChild)
    #print(report2)
    finalRep.to_csv("./GWAS/gwasTimp/" + str(fileName1) + ".txt" , sep="\t")

    takeVarInfo = GwasData.drop_duplicates(subset='GWASdiseaseHit', keep="first")

    finalRep = pd.read_csv("./GWAS/gwasTimp/" + str(fileName1) + ".txt" , sep="\t",names=['GWASdiseaseHit','gwasLikelihood'])
    ...
    del New['Link']
    # print(New)
    New.to_csv("./ChildResults/FinalGW_" + str(fileName1), index=False, sep="\t")
